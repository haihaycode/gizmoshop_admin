<template>
  <div>
      ...
      <template #body>
        <tr v-for="(item, index) in formattedStaffList" :key="index" class="hover:bg-gray-300">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
            <i @click="handleOpenModalSetting(item.id)" class='bx bx-cog text-xl'></i>
          </td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
    ...
    <ModalBox :isOpen="ModalSettingIsOpen" :closeModal="handleCloseModalSetting">
      <template #header>
...
export default {
  ...
  methods: {
    ...
    handleOpenModalSetting(accountId) {
      this.idAccountSelected = accountId;
      this.ModalSettingIsOpen = !this.ModalSettingIsOpen;
    },
    handleChangeRole() {
