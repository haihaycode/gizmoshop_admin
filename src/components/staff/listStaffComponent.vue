  ...
  <div>
    <TableComponent>
      ...
      <template #header>
        ...
        <th @click="changeSort('fullname')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Tên Nhân Viên <span v-html="getSortIcon('inventoryName')"></span></th>
        <th @click="changeSort('email')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Email <span v-html="getSortIcon('city')"></span></th>
        <th @click="changeSort('sdt')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Số điện thoại <span v-html="getSortIcon('district')"></span></th>
        <th @click="changeSort('birthday')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Năm sinh<span v-html="getSortIcon('ward')"></span></th>
        <th @click="changeSort('extra_info')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Thông tin thêm <span v-html="getSortIcon('longitude')"></span></th>
        <th @click="changeSort('extra_info')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Quyền <span v-html="getSortIcon('longitude')"></span></th>
        <th @click="changeSort('formattedCreateAt')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Ngày khởi tạo <span v-html="getSortIcon('latitude')"></span></th>
        <th @click="changeSort('deleted')" class="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Trạng thái <span v-html="getSortIcon('active')"></span></th>
      </template>

      <!-- Body Slot -->
      <template #body>
        <tr v-for="(item, index) in formattedStaffList" :key="index" class="hover:bg-gray-300" @click="updateInventoryModal(item.id)">
